...
class Solution:
    def numDifferentIntegers(self, word: str) -> int:
        ...
        for digit_end in range(len(word)):
            if digit_start == -1 and word[digit_end].isdigit():
                digit_start = digit_end
            elif digit_start != -1 and word[digit_end].isalpha():
                digit = int(word[digit_start:digit_end])
                unique_integers.add(digit)
                digit_start = -1
        
        if digit_start != -1:
            digit = int(word[digit_start:len(word)])
            unique_integers.add(digit)
