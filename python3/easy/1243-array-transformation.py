...
class Solution:
    def transformArray(self, arr: List[int]) -> List[int]:
        previous_day_arr = list(arr)
        while True:
            operations_performed = False
            current_day_arr = list(previous_day_arr)
            for i in range(1, len(previous_day_arr) - 1):
                if (
                    previous_day_arr[i] < previous_day_arr[i - 1]
                    and previous_day_arr[i] < previous_day_arr[i + 1]
                ):
                    current_day_arr[i] += 1
                    ...
                    current_day_arr[i] -= 1
                    operations_performed = True
            previous_day_arr = current_day_arr
            if not operations_performed:
                break

        return previous_day_arr
