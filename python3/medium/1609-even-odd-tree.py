    ...
    def isEvenOddTree(self, root: Optional[TreeNode]) -> bool:
        ...
        while q:
            ...
            for _ in range(len(q)):
                node = q.popleft()
                if (is_even and (node.val % 2 != 0 or (previous_node_val and node.val >= previous_node_val))) or (not is_even and (node.val % 2 == 0 or (previous_node_val and node.val <= previous_node_val))):
                    return False
                if node.left:
                    q.append(node.left)
                if node.right:
